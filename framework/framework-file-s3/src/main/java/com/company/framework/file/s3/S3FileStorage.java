...
public class S3FileStorage implements FileStorage {
    ...
    public InputStream load(String storedPath) {
        try {
            return s3.getObject(GetObjectRequest.builder().bucket(bucket).key(storedPath).build());
    ...
    public void delete(String storedPath) {
        s3.deleteObject(DeleteObjectRequest.builder().bucket(bucket).key(storedPath).build());
    }

    @Override
    public String type() { return "s3"; }
