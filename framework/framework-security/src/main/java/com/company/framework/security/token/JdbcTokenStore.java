...
public class JdbcTokenStore implements TokenStore {
    ...
    public void saveRefresh(String refreshToken, RefreshEntry entry, Duration ttl) {
        ...
        jdbc.update("INSERT INTO refresh_tokens (token, user_id, roles, expires_at) VALUES (?, ?, ?, ?)",
                refreshToken, entry.userId(), String.join(",", entry.roles()),
        ...
        List<RefreshEntry> rows = jdbc.query(
                ...
                (rs, n) -> {
                    ...
                    List<String> roleList = (roles == null || roles.isBlank())
                            ? List.of() : Arrays.asList(roles.split(","));
                    return new RefreshEntry(rs.getString("user_id"), roleList);
                },
                refreshToken, Timestamp.from(Instant.now()));
    ...
    public void blacklist(String jti, Duration ttl) {
        ...
        jdbc.update("INSERT INTO token_blacklist (jti, expires_at) VALUES (?, ?)",
                jti, Timestamp.from(Instant.now().plus(ttl)));
    }
    ...
    public boolean isBlacklisted(String jti) {
        Integer cnt = jdbc.queryForObject(
                ...
                Integer.class, jti, Timestamp.from(Instant.now()));
