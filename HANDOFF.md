...
- **다음 우선순위**: (1) **규제특화 잔여**(pki/hsm/recon/egov, 해당 사업만). (2) **그릇 정비** — 게이트웨이(폴백·CORS·rate-limit)·k8s(redis/secret/멀티서비스, observability ServiceMonitor 실배포)·CI-CD 멀티서비스화. (3) (선택) 멱등 재생 페이로드 지문(payload hash)로 "같은 키+다른 본문→422" 강제(IETF 권고, 현재 키만 판정). (상세 순서는 `docs/FRAMEWORK_MODULES.md` 4절)
