...
- **다음 우선순위**: (1) **규제특화 잔여**(pki/hsm/recon/egov, 해당 사업만). (2) (선택) idempotency 에 `JdbcIdempotencyStore` 추가(현재 memory/redis만). (3) **그릇 정비** — 게이트웨이(폴백·CORS·rate-limit)·k8s(redis/secret/멀티서비스, observability ServiceMonitor 실배포)·CI-CD 멀티서비스화. (상세 순서는 `docs/FRAMEWORK_MODULES.md` 4절)
