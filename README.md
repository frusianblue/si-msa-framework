...
services/
  ...
  auth-server         OAuth2/OIDC Authorization Server(OP) — 외부/그룹사 위임 토큰 발급(:9000) — 선택 배포
deploy/
...
- **오토컨피그 로딩 테스트 주의**: `ApplicationContextRunner`로 오토컨피그를 로딩하는 테스트는 그 설정 클래스를 리플렉션으로 introspect하며 **모든 `@Bean` 메서드의 파라미터/반환 타입을 로드**한다(`@ConditionalOnClass`와 무관). 따라서 해당 모듈 `build.gradle`의 `compileOnly` 의존(web/jdbc/redis 등)을 **전부 `testImplementation`으로 다시 선언**해야 컨텍스트가 뜬다. 누락 시 `NoClassDefFoundError → Failed to parse configuration class`. (운영은 Boot가 ASM으로 읽어 영향 없음 — 테스트 전용 함정. 예: `framework-mfa`는 web+jdbc+data-redis 3종 재선언.) **2026-06-03: 무테스트였던 라이브러리 모듈 14개에 로딩/토글 스모크를 추가해 무테스트 라이브러리 모듈 0**(서블릿 한정 오토컨피그는 `WebApplicationContextRunner`, `@MapperScan`+MyBatis 결합 모듈은 enabled 가 DB를 요구해 `enabled=false` 백오프까지 검증). 모두 `testImplementation`이라 런타임/배포 영향 없음.
- **콘솔 한글 인코딩**: 테스트 워커/데몬은 루트 `build.gradle`(Test `jvmArgs`)·`gradle.properties`(`org.gradle.jvmargs`)에서 UTF-8 고정. 다만 콘솔에 최종 렌더하는 건 `gradlew` 클라이언트 JVM이라, Windows에서 `@DisplayName` 한글이 깨지면 셸에 `export GRADLE_OPTS="-Dfile.encoding=UTF-8 -Dstdout.encoding=UTF-8 -Dstderr.encoding=UTF-8"`를 추가한다(데몬 설정 변경 시 `./gradlew --stop` 후 재실행).
...
> **설정값(시크릿) 암호화**: yaml 의 패스워드/시크릿을 `ENC(...)` 로 두면 기동 시 자동 복호화한다. 마스터키 = `framework.crypto.aes-secret`(운영 `AES_SECRET`, **항상 평문 주입** — 닭-달걀이라 마스터키 자신은 `ENC()` 불가). 토큰 생성: `AES_SECRET=... java -cp <cp> com.company.framework.core.crypto.CryptoCli encrypt '평문'` → `ENC(...)` 출력. 토글 `framework.crypto.config-decryption.enabled`(기본 on; `ENC()` 가 없으면 무동작). 기존 `AesCryptoService`(AES-GCM) 재사용, 신규 의존성 0. 키가 틀리거나 마스터키 미주입 시 **기동 실패**(조용히 통과 안 함). 상세 `docs/archive/NEXT_YAML_PASSWORD_ENCRYPTION.md`.
...
dependencies {
    ...
    implementation project(':framework:framework-file-sftp')   // SFTP 원격 저장(MINA SSHD 위임·Range 지원·연결 풀/키 회전 옵트인, storage.type=sftp) — sshd-core/sshd-sftp 전이
    implementation project(':framework:framework-qr')           // QR 코드 생성(ZXing core 인코딩 + JDK ImageIO 렌더링, framework.qr.enabled) — zxing-core 전이(의존성 1개)
}
```
...
### framework-cache-redis (분산 캐시 — 파드 간 공유)
k8s 다중 파드가 **한 캐시를 공유**하도록 core 의 로컬 Caffeine `CacheManager` 를 Redis 로 **대체**한다(끄면 기본 Caffeine). core 가 캐시 매니저를 항상 등록(`matchIfMissing`)하므로 본 모듈은 `@AutoConfiguration(before=CacheAutoConfiguration)` 로 먼저 떠 core 를 백오프시킨다. 신규 외부 의존성 0(spring-data-redis 는 `compileOnly`).
```yaml
framework:
  cache:
    redis:
      enabled: true            # 끄면(기본) core Caffeine 로컬 캐시
      time-to-live: 10m
      key-prefix: ""
      cache-null-values: true
      ttls: { commonCode: 1h, userProfile: 5m }   # 캐시별 TTL override
spring:
  data:
    redis: { host: redis, port: 6379 }            # 호스트 앱이 Redis 연결 제공
```
- 사용처 코드는 그대로 — `@Cacheable("이름")` 만(로컬/분산은 설정으로 전환, `@EnableCaching` 은 core 가 켬). 캐시 값은 `Serializable`(값=JDK 직렬화·키=String).
- **Jackson 3 규약**: Jackson2 `GenericJackson2JsonRedisSerializer` 는 안 쓴다. JSON 직렬화가 필요하면 앱이 `RedisCacheConfiguration` 빈을 직접 등록(본 모듈 기본은 `@ConditionalOnMissingBean` 이라 앱 빈 우선). 상세 `framework/framework-cache-redis/README.md`.

### framework-log-masking (개인정보 로그 마스킹)
core `MaskingUtils`(값 단위)의 보완으로, **자유 텍스트 로그**에 섞여 흘러든 PII(주민/카드/휴대폰/이메일, 계좌는 오탐 위험으로 기본 off)를 정규식으로 탐지해 가린다. 신규 외부 의존성 0(logback-classic 은 Boot 기본 로깅).
```yaml
framework:
  log-masking:
    enabled: true
...
```
```java
// 1차(확실): SensitiveDataMasker 빈 명시 호출 — 구조화 로그까지 커버
log.info("AUDIT payload={}", masker.mask(dto.toString()));
```
```xml
<!-- 2차(방어망): logback-spring.xml 에 include 후 root 를 마스킹 appender 로 -->
