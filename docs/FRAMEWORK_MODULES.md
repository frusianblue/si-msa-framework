...
- ⏭️ **다음 후보**: **인증 로드맵 3) SSO — B-SAML**(SAML 2.0 SP `framework-saml-sp` 신설; A 중앙로그아웃·B-OIDC 완료 → 다음은 SAML, ⚠️ OpenSAML 전이=첫 "의존성 0" 예외 — `docs/NEXT_SSO.md` §5) · (이후) C) Authorization Server·4) Passwordless · (devops) CI 게이트 + 멀티모듈 jacoco 집계 · **그릇 정비**(게이트웨이 런타임 점검·k8s 멀티서비스/CI-CD) · (선택) QR 코드(zxing opt-in)·SFTP 후속(연결 풀·키 회전)·아카이빙 tar/tar.gz·규제특화 잔여(pki/hsm/recon/egov)
...
| ⏭️ framework-saml-sp | **SAML 2.0 SP**(다음 세션) — 외부 SAML IdP 연동(Spring Security SAML2 SP) → `SamlUserResolver` 매핑 → 자체 JWT. ⚠️ OpenSAML 전이=첫 "외부 의존성 0" 예외, 멀티파드 `Saml2AuthenticationRequestRepository`(redis). 설계 `docs/NEXT_SSO.md` §5 | `framework.saml-sp.enabled` + `registrations.<id>.{idp-metadata-uri\|idp cert/sso-url, sp.entity-id/acs}` | [선택] | 공공/대기업 SSO |
