...
| framework-security-rbac-mybatis | **RBAC 영속의 MyBatis 어댑터**(2026-06-07) — 보안-영속 결합 분리. 코어(framework-security)의 RBAC 포트(`ResourceMetadataProvider`/`MenuProvider`)를 MyBatis 로 구현(`SecurityMapper` FQN 유지) + 감사 브리지(`@Primary CurrentUserProvider`) 이전. **코어는 더 이상 MyBatis/DataSource 강제 안 함** — 인증만 쓰는 서비스는 의존 0개·DataSource 불필요, DB 동적 인가/메뉴 쓰는 서비스만 의존 한 줄. `dynamic-authorization=true`(기본)인데 어댑터 없으면 **부팅 fail-fast**(조용한 인가 무력화 차단). 미래 확장=`-rbac-jpa`/`-rbac-jdbc` 어댑터만. 사용 `framework/framework-security-rbac-mybatis/README.md` | (코어 토글 그대로) `framework.security.dynamic-authorization`/`menu` | [선택] | 공통 |
