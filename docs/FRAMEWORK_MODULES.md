...
- ⏭️ **다음 후보**: **★ 파일 일괄처리**(`framework-file-batch`, 설계서 `docs/NEXT_FILE_BATCH_PROCESSING.md` — 다음 세션 최우선) · (devops) CI 게이트 + 멀티모듈 jacoco 집계 · **그릇 정비**(게이트웨이 런타임 점검·k8s 멀티서비스/CI-CD) · (선택) 아카이빙 후속 tar/tar.gz·RetryUtils · 규제특화 잔여(pki/hsm/recon/egov) · (선택) saga 단계별 타임아웃/보상 재시도·실DB 통합테스트
...
| ⏭️ framework-file-batch | **파일 일괄처리(다음 착수)** — 여러 파일에 동일 작업(이름변경/이미지변환/압축)을 한꺼번에. `BatchFileOperation` SPI + `FileBatchProcessor`(부분실패 격리+결과수집·Java21 가상스레드 병렬·드라이런). image/archive 위임(compileOnly). **외부 의존성 0 예정.** 설계 `docs/NEXT_FILE_BATCH_PROCESSING.md` | `framework.file-batch.enabled` | [선택] | 공통 |
