...
| **유형 카탈로그 앱** | `examples/batch-types/` | **처리 방식·DB 리더/라이터 종류 6종**(2026-06-07): `multiThreadedChunkJob`(멀티스레드 청크+JdbcPagingItemReader, 커서 불가→페이징+saveState(false))·`partitionedJob`(로컬 파티셔닝 `Partitioner`+@StepScope 워커)·`flowControlJob`(조건분기 on/to + 병렬 split)·`compositeProcessorJob`(`CompositeItemProcessor` 검증→보강 체인)·`mybatisJob`(MyBatisPagingItemReader→MyBatisBatchItemWriter, **mybatis-spring 4.0.0**)·`storedProcedureJob`(`StoredProcedureItemReader` PG refcursor → `itemPreparedStatementSetter` record 라이터). 공유 소스 `txn` 12행, 잡별 결과 테이블. paging/partition/flow/composite/storedproc/mybatis API 전부 v6.0.3·mybatis-spring 4.0.0 raw 실측 |
