...
3. **GitOps 자산** — `deploy/argocd/`: AppProject(`projects/si-msa.yaml`) + prod Application(`apps/si-msa-prod.yaml`, source=`overlays/prod`, targetRevision=`master`, dest=`kind-svc`, sync 자동+selfHeal+prune) + app-of-apps(`bootstrap.yaml`). **🟡 산출 완료(2026-06-08) · 받는 쪽 실행 대기** — `20-gitops-bootstrap`(commit-first: ArgoCD=git 진실)·`92-verify-gitops` G8~G10. 파드 ImagePullBackOff=fail-loud(sentinel `__GITSHA__`, 4단계 promote+Harbor 후 green).
