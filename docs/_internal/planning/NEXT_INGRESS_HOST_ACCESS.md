...
> 결정(2026-06-07 세션3 말미): 호스트 접속을 **port-forward 졸업**시킨다.
> 방식 **B** = 클러스터 재생성 + `extraPortMappings` + ingress-nginx + Ingress(`harbor.local`/`jenkins.local`).
> 배경: standalone kind 는 docker-desktop kind 와 달리 LB→localhost 자동게시가 없고, 현 `kind-sanity`
>   `kind-config.yaml` 엔 `extraPortMappings` 가 없어 노드 포트가 호스트로 안 나온다. extraPortMappings 는
>   **클러스터 생성 시점 고정** → 재생성 불가피.
