...
<!-- 갱신 시작 -->
## 이번 세션 한 줄 요약
**🟡 prod GitOps 4단계(Harbor hub + bash promote) 자산 작성·정적검증 완료 — 받는 쪽 실측(G11~G13) 대기(2026-06-08, devops · kind 멀티클러스터).** 빌드 주체 = **호스트 docker**(Chae 결정, Kaniko 아님 — "흐름부터 증명" 목적, 미해결 Kaniko 멀티빌드 회피). 신규 자산 3 (`deploy/k8s/prod-kind/`): **`30-harbor-hub-install.sh`**(standalone `08-harbor-install` 의 멀티클러스터 적응판 — kind-cicd 에 Harbor Helm[standalone `harbor-values.yaml` 단일소스 재사용] + si-msa public 프로젝트. cicd 노드 certs.d/CoreDNS **불요**: push=호스트 docker→127.0.0.1:80→ingress, pull=kind-svc 노드[03-cross-trust 배선]. kind-svc 신뢰 유효성 재확인 포함). **`40-promote.sh`**(① 호스트 docker `Dockerfile.build` 4서비스 `:<sha>` 빌드[builder SERVICE 무관 → 1회 컴파일 재사용] → ② `harbor.local/si-msa/<svc>:<sha>` push → ③ `kustomize edit set image .../<svc>=harbor.local/si-msa/<svc>:<sha>`[placeholder newName + sha newTag, 멱등] → ④ **git commit/push**[prod 반전 — dev "되커밋 X"와 정반대] → ⑤ ArgoCD `refresh=hard`. 0단계 워킹트리 clean 가드). **`41-verify-promote.sh`**(G11 Harbor repo `:<sha>` 아티팩트 · G12 핀[newTag sentinel 소거+newName harbor]+커밋·푸시 · G13 si-msa-prod Synced/Healthy+kind-svc 파드 green). 정적검증: bash -n PASS, PyYAML 로 kustomize 핀 후 형식 시뮬레이션 → G12 grep 패턴 검증(주석 `__GITSHA__` 오탐 회피 = `newTag: __GITSHA__` 한정). 문서 캐스케이드: prod-kind README §4 추가 · spec §0/§3-4 진행 표기 · PITFALLS §9 +2(placeholder 핀 = local 함정 prod/harbor 판 · 검증 grep 오탐). **다음 = 받는 쪽 실측**(30→40→대기→41) 후 결과 보고 → 그린이면 5단계.
...
2. **실측**: `bash 30-harbor-hub-install.sh` → `bash 40-promote.sh`(자동 commit/push 주의) → (sync/pull/startup 대기) → `bash 41-verify-promote.sh`(G11~G13).
...
- **prod 핀은 newName 까지 박는다**: prod overlay image name=`registry.example.com/...`(placeholder). newTag 만 바꾸면 가짜 레지스트리 pull=ImagePullBackOff(local overlay 함정의 prod/harbor 판). `kustomize edit set image NAME=NEWNAME:TAG` 로 newName+newTag 동시.
