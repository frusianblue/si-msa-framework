...
- ⚠️ **LOG_DIR 수정은 받는 쪽 재기동 확인 필요** — `up -d`(env 변경이라 user/admin 재생성, --build 불요) 후 `docker compose ps` 에서 user/admin 이 `(healthy)` 로 떠야 함. 이때 비로소 **user/admin prod 첫 Flyway(sidb/admindb)** 구간까지 통과 확인됨. 혹시 admin 이 `admindb 없음`류로 또 막히면 `down -v` 후 재기동(초기 `pgdata` 볼륨이 admindb 추가 전 생성됐을 가능성).
