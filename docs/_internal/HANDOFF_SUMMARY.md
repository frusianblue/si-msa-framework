...
- ⚠️ **LOG_DIR+업로드경로 수정 후 재기동 확인 필요** — `up -d`(env 변경, --build 불요) 후 `docker compose ps` 에서 user/admin `(healthy)`. user/admin 은 부팅 순서상 logging→file storage→**datasource/Flyway** 로 진행하므로, 다음 잠재 관문 = **prod 첫 Flyway(sidb/admindb)**. admin 이 `admindb 없음`류로 막히면 `down -v` 후 재기동(초기 `pgdata` 볼륨이 admindb 추가 전 생성됐을 가능성).
