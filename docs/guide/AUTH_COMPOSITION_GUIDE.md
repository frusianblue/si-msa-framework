...
| Passkey / WebAuthn(FIDO2) | `framework-webauthn` `enabled` + `spring-security-webauthn` + 앱 `UserDetailsService` | 🟢 1차 슬라이스(2026-06-05): SS7 네이티브 `http.webAuthn()` 래핑 → 세션+CSRF 전용 체인 ceremony → 토큰교환으로 자체 JWT. 저장소 memory\|jdbc. 브라우저 ceremony 실서명은 받는 쪽 검증 |
...
| 후보 | 무엇 | 신설 모듈(예) |
|---|---|---|
| ~~Passkey / WebAuthn~~ | ✅ **1차 구현됨(2026-06-05)** — `framework-webauthn`(SS7 네이티브 `http.webAuthn()` 래핑, 전용 세션+CSRF 체인 → 자체 JWT 교환). 사용 `framework/framework-webauthn/README.md`. (후속: 2차 MFA factor 연계·passkey 관리 UX) | `framework-webauthn` |
