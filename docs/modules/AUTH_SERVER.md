...
- **▶ 토큰 발급 라운드트립 통합테스트**: demo-web authorization_code+PKCE, demo-service client_credentials → 게이트웨이 → user-service zero-trust 재검증.
  ...
  - **✅ confidential `demo-rp` 전체 콜백 흐름 완료(2026-06-06)**: `OAuthLoginService.callback()` 이 `OAuthClient.exchangeCodeForTokens`(`client_secret_post`)로 코드 교환 → id_token 검증 → userinfo → 자체 토큰. `e2e/OidcRpFullCallbackTest`(양성 1 + 음성 3). `demo-rp` 는 `LocalDemo`·`SmokeClientSeeder` 양쪽 등록(PKCE 불요, consent 미요구). 상세 [`../_internal/planning/NEXT_RP_IDTOKEN_LINK.md`](../_internal/planning/NEXT_RP_IDTOKEN_LINK.md) §B(ARCHIVED).
