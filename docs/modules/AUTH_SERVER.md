...
- 켜면(=`FRAMEWORK_AUTH_SEED_SMOKE_CLIENT=true`): `demo-web`(public+PKCE)·`demo-service`(client_credentials)를 **`RegisteredClientRepository.save()`** 로 멱등 등록한다.
...
```bash
# 1) 시더가 포함된 auth-server 이미지 재빌드(Docker Desktop kind = registry-mirror 자동노출, 적재 불요)
#    이미지는 compose 빌드 정의(deploy/docker/Dockerfile.build, SERVICE 인자, context=repo root)로 si-msa/auth-server:local 생성.
docker compose -f deploy/compose/docker-compose.yml build auth-server
...
# 3) authorization_code+PKCE: 브라우저로 /oauth2/authorize?client_id=demo-web... → tester/Test1234! 로그인(DbAuthenticator)
...
- **▶ 토큰 발급 라운드트립 통합테스트**: demo-web authorization_code+PKCE, demo-service client_credentials → 게이트웨이 → user-service zero-trust 재검증.
  ...
  - **prod 경로(DbAuthenticator, smoke 시더) ▶ 코드/테스트 전달(2026-06-06)**: `SmokeClientSeeder`(옵트인 `framework.auth.seed-smoke-client`) + `SmokeClientDbAuthFlowTest`(`@ActiveProfiles("smoketest")`) — `tester`/`Test1234!`(authdb `app_user`) 폼 로그인 → authorization_code+PKCE → access/id_token(sub=`tester`). overlays/local 의 `auth-server-config` 가 플래그를 on. **받는 쪽 검증 필요**(작성환경 Central 차단 → §7 규약): `./gradlew :services:auth-server:test --tests '*SmokeClientDbAuthFlowTest'` + kind 수동 절차(§6.5). 상세 [`../_internal/planning/NEXT_KIND_AUTH_TOKEN_FLOW.md`](../_internal/planning/NEXT_KIND_AUTH_TOKEN_FLOW.md).
  - (백로그) confidential `demo-rp` 전체 콜백 흐름(`OAuthClient.exchangeCodeForTokens`, `client_secret_post`) — [`../_internal/planning/NEXT_RP_IDTOKEN_LINK.md`](../_internal/planning/NEXT_RP_IDTOKEN_LINK.md) §B.
