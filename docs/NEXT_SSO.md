...
>
> **▶ 후속 완료(2026-06-04): 게이트웨이 이중 발급기 정합.** 정합 지점=게이트웨이(1차 canonical 검증자). `services/gateway` 가 토큰 `iss` 로 분기 — AS issuer 면 `{issuer}/oauth2/jwks` 로 RS256 검증(`GatewayJwksTokenVerifier`, `JwksKeyResolver` 패턴을 servlet 의존 없이 jjwt `keyLocator` 로 자립 재현), 내부면 자체 JWT(HMAC). AS 토큰은 jti 블랙리스트 미조회(폐기=AS `/oauth2/revoke`, §4). JWKS 조회(블로킹)는 AS 토큰일 때만 `boundedElastic` 오프로드. 옵트인 `gateway.auth.authorization-server.enabled`(기본 off=도입 전 동작). **프레임워크/다운스트림 무변경**(게이트웨이가 헤더 주입 → `framework-context` 그대로 수용). 상세 `docs/modules/GATEWAY_EDGE_AUTH.md`(이중 발급기)·`AUTH_SERVER.md` §4/§8. **남은 후속**: 서명키 회전 스케줄러 · (선택) 다운스트림 servlet zero-trust 재검증(framework-security 변경 수반, 별도 드롭).
