...
5. **다운스트림 AS 토큰 재검증 (구현됨, 2026-06-04).** 과거엔 `user-service` 가 자체 JWT만 재검증하고 AS 토큰(RS256)은 게이트웨이에만 의존했으나, 이제 `framework-security` 가 `edge-trust.mode=zero-trust` + `resource-server.enabled=true` 에서 **AS 토큰까지 무신뢰 재검증**한다(§6.4, §7.3). VM 등 격리가 약한 환경의 완전한 A2 를 지원.
...
### 6.4 (구현됨) 다운스트림 zero-trust 재검증 — 완전한 A2 ✅

`framework-context`-only 서비스도, `framework-security` 를 켠 서비스도 `Authorization` Bearer 를 **직접 이중 issuer 재검증**하도록 `framework-security`(servlet)에 검증기를 이식했다(2026-06-04). 게이트웨이의 `GatewayTokenAuthenticator`/`GatewayJwksTokenVerifier` 와 같은 로직의 servlet 판이다.

- 신규 `framework-security`: `jwt/ResourceServerJwtVerifier`(AS RS256/JWKS, `JwksKeyResolver` 패턴 자립 재현, HS 거부, `iss`/`aud`/`sub` 검증) · `jwt/DownstreamTokenAuthenticator`(iss 라우팅 → Spring `Authentication`) · `jwt/TokenIssuerKind`.
- `jwt/JwtAuthenticationFilter` 가 `framework.security.edge-trust.mode` 로 분기(§7.3): `zero-trust` 는 Bearer 재검증(자체 JWT + AS), `gateway-headers` 는 주입 헤더 신뢰.
- AS 토큰은 자체 jti 블랙리스트와 혼용 안 함(§4) — INTERNAL 만 블랙리스트 조회.

전환 후에도 NetworkPolicy(C1)는 심층 방어로 유지한다(배타가 아니라 겹겹이).
...
### 7.3 환경별 선택을 코드로 분기하기 — `edge-trust.mode` (구현됨)

`framework-security` 가 `framework.security.edge-trust.mode` 로 다운스트림 신뢰 자세를 분기한다(2026-06-04 구현).
K8s/VM은 `local|dev|prod` 프로파일과 **직교**(dev를 VM에도 K8s에도 올릴 수 있음)하므로, 이 프로젝트의 dev/prod 환경변수 주입
컨벤션에 맞춰 **env 로 주입**한다(원하면 전용 오버레이 프로파일에 고정해도 된다). 같은 산출물(jar/이미지)이 환경에 맞는 자세로 뜬다.
...
framework:
  security:
    ...
    resource-server:                        # AS(OP) RS256/JWKS 토큰까지 zero-trust 재검증(기본 off)
...
```bash
# K8s (NetworkPolicy 로 게이트웨이 우회 불가 보장) → 완화: 헤더 신뢰, Bearer 재검증 생략
EDGE_TRUST_MODE=gateway-headers

# VM (격리 약함) → 기본 zero-trust 유지 + AS 토큰까지 재검증
EDGE_TRUST_MODE=zero-trust
...
|-------------------|-----------------|------------------|
| `gateway-headers` | 주입된 `X-User-*` 를 그대로 신뢰(Bearer 재검증 생략 → AS 토큰도 게이트웨이 검증 결과로 자동 수용) | 게이트웨이가 박은 헤더 |
| `zero-trust` | `Authorization` Bearer 를 **직접 재검증**(자체 JWT HMAC + AS RS256/JWKS) 후에만 인증; 검증 없는 `X-User-*` 는 authn 근거로 불사용 | 로컬 재검증 결과 |

지킬 원칙 둘:

1. **기본값은 엄격(zero-trust).** 완화(`gateway-headers`)는 "네트워크 격리를 확인했다"는 **의도적 옵트인**. 잘못 켜면 곧 구멍이라 "기본 안전" 철학과 같은 결.
2. **AS 토큰 zero-trust 재검증은 `resource-server.enabled=true` 가 필요**(없으면 AS 토큰은 INTERNAL 로 분기→HMAC 실패→401). 자체 JWT만 쓰면 켤 필요 없음.

> **참고**: `framework-context`-only 서비스(보안 모듈 없음)는 본질적으로 헤더 신뢰형이다. 그런 서비스에 zero-trust 가 필요하면
> `framework-security` 를 함께 켜야 한다(인증 권한이 보안 체인에 있다).
